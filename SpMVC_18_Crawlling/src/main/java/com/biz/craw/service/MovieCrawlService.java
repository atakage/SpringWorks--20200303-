...
import com.biz.craw.persistence.NaverDao;
...
public class MovieCrawlService {
	 ...
	 *  url, title이 들어있는 tag, image가 들어있는 tag, rankList를 가져오기 위한 tag묶음 정보를 변수로 선언
	 *  
	 */
	 ...
	 * Unix에서는 일정한 시간(년, 월, 일, 시, 분, 초)를 지정해서 어떤 일을 정기적으로 수행할 때
	 * cron tab이라는 기능을 이용해서 작업을 지정할 수 있음(초, 분, 시, 일, 월, 요일, 년)
	 */
	...
	public List<NaverMovieVO> movieRankGet() {
		...
		// jsoup의 Document 클래스를 사용하여 DOM 형식의 Document를 만듦
		...
		// DOM rankListTag 문자열을 기준으로 List 추출
		...
		// rankList box들 중 상위 1부터 10번까지만 가져오기 수행
		for(int i = 0; i < 10; i++) {
			...
			// div.thum a img의 src 속성값 추출(영화 이미지 URL)
			String imgURL = imgList.get(i).attr("src");
			
			
			naverList.add(NaverMovieVO.builder().m_title(title).m_rank(i+1).m_detail_url(detailURL).m_image_url(imgURL).build());
