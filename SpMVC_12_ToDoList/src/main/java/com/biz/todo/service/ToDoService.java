package com.biz.todo.service;

import java.util.List;

import org.springframework.stereotype.Service;

import com.biz.todo.domain.ToDoList;

@Service
public interface ToDoService {							
	
	/*
	 * 인터페이스 사용 이유
	 * 1. 어떤 서비스를 구현할 것인가의 설계도
	 * 2. 여러 팀원이 분담하여 프로젝트를 구현할 때 꼭 필요한 method를 빼먹지 않고 구현할 수 있도록 하는 Guide 역할
	 * 3. 자바의 다형성 원리를 이용하여, 버전업을 했을 때 기존에 사용하던 코드를 최소한으로 변경하여 효과를 낼 수 있도록 하는 목적
	 * 4. 클래스를 상속받을 때 코드가 난해해지는 것을 방지하며 클래스와 클래스 사이에서 서로 연관도를 낮추어 유연한 관계를 설정하기도 함
	 * 5. 인터페이스는 기획자, PM등이 미리 설정을 해 두어야 함
	 * 
	 * 
	 * 
	 */
	
	public List<ToDoList> selectAll();
	public ToDoList findBySeq(long tdSeq);
	public List<ToDoList> findBySubject(String tdSubject);
	public int insert(ToDoList toDoList);
	public int update(ToDoList toDoList);
	public int delete(ToDoList toDoList);
	public int complete(String strSeq, String tdComplete);
	public int alaram(String strSeq, String tdAlaram);

}
