...
public class ToDoListSQL {
	...
	public final static String complete_sql = "UPDATE tbl_todolist SET td_complete = DECODE(td_complete, 'Y', 'N', 'Y') WHERE td_seq = #{td_seq}";
	...
	public String insert_sql(final ToDoList toDoList) {
		...
		return new SQL() {{
				...
				INTO_VALUES("#{td_alaram, jdbcType=VARCHAR}");
	...
	public String alaram_sql() {
		...
		/*
		 * SQL Class mybatis 3.4이상에서 사용되는 동적 쿼리를 지원하는 클래스
		 * 
		 */
		...
		sql.SET("td_alaram = DECODE(td_alaram, 'Y', 'N', 'Y')");
	...
	public String update_sql() {
		...
		return new SQL() {{
			...
			SET("td_alaram	=#{td_alaram, jdbcType=VARCHAR}");
