...
public class EMSController {
	 ...
	 * 이 method를 실행하여 emialVO를 만듦
	 * 
	 * 하지만 현재 상태에서 한번이라도 이 method가 호출되어 emailVO가 생성된 상태라면 다시 method가 수행되지 않음
	 * 
	 * 
	 * */
	...
	public String input(@ModelAttribute("emailVO") EmailVO emailVO, Model model, SessionStatus status, HttpSession httpSession) {
		...
		// naver에서 가져온 email이 실제 로그인한 email이 아니고 복구용 email이기 때문에 실제 email보내기가 안 될 수도
	...
	public String update(@ModelAttribute("emailVO") EmailVO emailVO, @PathVariable("ems_seq") String ems_seq, Model model) {
		 ...
		 model.addAttribute("emailVO", emailVO);
		 model.addAttribute("ems_seq", ems_seq);
	...
	public String updateNext(@ModelAttribute("emailVO") EmailVO emailVO, @PathVariable("ems_seq") String ems_seq) { 
		
		// db에 업데이ㅡ
		long ems_seqL = Long.valueOf(ems_seq);
		
		emailVO.setEms_seq(ems_seqL);
