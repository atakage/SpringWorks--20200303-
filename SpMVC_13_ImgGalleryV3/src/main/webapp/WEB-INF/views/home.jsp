...
var contextCallBack = function(key, options){
	...
	*/
		...
		if(key == "delete"){
			if(confirm(id + "이미지를 삭제할까여")){
				...
				$.ajax({
					...
					url: "${rootPath}/image/delete2", data:{img_seq : id}, type:'POST',
					success : function(result){
				...
				*/
				...
				// 현재 클릭된(div) 태그가 포함하고 있는 tag들 중 img 태그를 찾아 src값 가져옴(이미지 이름)
				let img_src = $(this).find('img').attr('src')
				
				//전체 src값에서 파일이름만 추출하기 위해 / 문자 기준으로 분해
				// 분해된 배열의 제일 마지막 값을 가져오면 파일이름만 획득
				...
				$.post('${rootPath}/rest/main_image', {img_pcode : img_pcode, img_file: img_file},
	...
	$.contextMenu({
		selector: '.img_view', items: {
			
			'loading': {name:'g확대보기', icon: 'loading'},
			'copy': {name:'다운로드', icon: 'copy'},
			'delete': {name:'삭제', icon: 'delete'},
			'main': {name:'대표이미지설정', icon: 'as fa-check-circle'}
