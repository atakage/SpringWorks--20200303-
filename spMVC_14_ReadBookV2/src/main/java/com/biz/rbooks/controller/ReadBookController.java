...
public class ReadBookController {
	...
	public String insert(Model model) {
		...
		LocalDate localDate = LocalDate.now();	// java 1.8 이상에서 사용하는 날짜 클래스		
		...
		String curTime = localTime.format(dtf).toString();
		
		readBookVO.setRb_date(curDate);
		readBookVO.setRb_stime(curTime);
		
		model.addAttribute("rBookVO", readBookVO);
	...
	public String insert(ReadBookVO rbookVO) {
		...
		int ret = readBookService.insert(rbookVO);
	...
	public String view(@PathVariable("rb_seq") long rb_seq, Model model) {
		...
		List<ReadBookVO> rBookList = readBookService.findByBCode(b_code);
	...
	public String update(@ModelAttribute ReadBookVO rBookVO) {
		...
		int ret = readBookService.update(rBookVO);
